                                ...
                                TABLE_LIST *table_list)
{
  ...
  */
  ...
  else if ((alter_info->partition_flags & ALTER_PARTITION_ADD_FROM_TABLE))
  {
    ...
    if (write_log_drop_shadow_frm(lpt) ||
        ERROR_INJECT("add_partition_from_1") ||
        mysql_write_frm(lpt, WFRM_WRITE_SHADOW) ||
        ERROR_INJECT("add_partition_from_2") ||
        wait_while_table_is_used(thd, table, HA_EXTRA_NOT_USED) ||
        wait_while_table_is_used(thd, table_from,
                                 HA_EXTRA_PREPARE_FOR_RENAME) ||

        ERROR_INJECT("add_partition_from_3") ||
        write_log_add_change_partition(lpt) ||
        ERROR_INJECT("add_partition_from_4") ||
        mysql_change_partitions(lpt) ||
        ERROR_INJECT("add_partition_from_5") ||
        alter_close_table(lpt) ||
        ERROR_INJECT("add_partition_from_6") ||
        check_table_and_partition_compatibility(lpt) ||
        move_table_to_partition(lpt) ||
        ERROR_INJECT("add_partition_from_7") ||
        write_log_rename_frm(lpt) ||
        (action_completed= TRUE, FALSE) ||
        ERROR_INJECT("add_partition_from_8") ||
        (frm_install= TRUE, FALSE) ||
        mysql_write_frm(lpt, WFRM_INSTALL_SHADOW) ||
        log_partition_alter_to_ddl_log(lpt) ||
        (frm_install= FALSE, FALSE) ||
        ERROR_INJECT("add_partition_from_9") ||
        (write_log_completed(lpt, FALSE), FALSE) ||
        ((!thd->lex->no_write_to_binlog) &&
            (write_bin_log(thd, FALSE,
                           thd->query(), thd->query_length()), FALSE)) ||
                           ERROR_INJECT("add_partition_from_10"))
    {
